...
M = 1  # Rows
N = 2  # Columns
D = 2.0  # Distance between drones in meters

SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent

WORLD_NAME = "swarm_world" # Must match your running gz sim world name
BASE_SDF_PATH = PROJECT_ROOT / "external" / "ardupilot_gazebo" /"models"/"iris_with_gimbal"/"model.sdf"
GIMBAL_SDF_PATH = PROJECT_ROOT / "external" / "ardupilot_gazebo" / "models" / "gimbal_small_3d" / "model.sdf"

def generate_instance_sdf(instance_id, base_sdf, base_gimbal_sdf):
    # Gimbal SDF 
...
def spawn_swarm():
    ...
    for i in range(total_drones):
        ...
        sdf_file = generate_instance_sdf(i, BASE_SDF_PATH, GIMBAL_SDF_PATH)
        ...
        # Stagger the boot sequence so your CPU doesn't choke
 #       time.sleep(2.0)

    print("Spawning Success, Try to open the SITL")
    time.sleep(5.0)
    spawn_sitl(total_drones)

    print("Swarm deployment complete. Connect QGroundControl or MAVProxy.")

def spawn_sitl(total_drones):
    full_env = os.environ.copy()
    project_dir = os.path.expanduser("~/project_quadAIROS")

    for i in range(total_drones):
        run_dir = PROJECT_ROOT / "run" / f"agent_{i}" 
        run_dir.mkdir(parents=True, exist_ok=True)

        sitl_cmd = [
            str(PROJECT_ROOT/"external/ardupilot/Tools/autotest/sim_vehicle.py"),
            "-v", "ArduCopter",
            "-f", "gazebo-iris",
            "-I", str(i),
            "--sysid", str(i + 1),
